...
//         query ,
//         `${candidate.metadata.title}.${candidate.metadata.description}`,
//     ]);
//     try {
...
export const rerank = async (query, candidates) => {
    ...
    try {
        const response = await axios.post(
            url,
            {
                inputs: pairs.map(([source, target]) => ({
                    text: source,
                    text_pair: target,
                })),
                options: {
        ...
        console.warn(
            `Cross-encoder rerank failed, falling back to RRF order: ${error.message}`
        );

        return candidates
            .map((candidate) => ({
                id: candidate.id,
                score: Number(sigmoid((candidate.rrfScore ?? 0) * 10).toFixed(4)),
                metadata: candidate.metadata,
            }))
            .sort((a, b) => b.score - a.score);
    }
